...
\begin{itemize}
    \item \textbf{More than 24 hours before departure:} 80\% refund
    \item \textbf{24 hours hours before departure:} 0\% refund
\end{itemize}
...
\textbf{Flow:} Email/password input → Email validation → Password hashing → Account creation → Optional passenger profile linking.\\
\textbf{Rules:} BR-1, BR-2, BR-3

\subsubsection*{UC-2: Search Available Flights}
\textbf{Actor:} User (authenticated or guest)\\
\textbf{Flow:} Enter search criteria → Validate route → Query SCHEDULED flights → Calculate seat availability → Retrieve dynamic pricing → Display results.\\
\textbf{Rules:} BR-13, BR-14, BR-15, BR-16, BR-17, BR-18, BR-21

\subsubsection*{UC-3: Create Group Booking}
\textbf{Actor:} Registered User\\
\textbf{Flow:} Create PENDING booking → Assign lead user → Generate PNR → Add passengers → Select seats (same class) → Calculate individual prices → Proceed to payment.\\
\textbf{Rules:} BR-1, BR-2, BR-4, BR-5, BR-6, BR-7, BR-8, BR-9, BR-19, BR-20, BR-22, BR-23

\subsubsection*{UC-4: Complete Payment}
\textbf{Actor:} Lead User\\
\textbf{Flow:} Process payment → Update status to CONFIRMED → Generate tickets → Send confirmation.\\
\textbf{Rules:} BR-5, BR-6

\subsubsection*{UC-5: Cancel Booking}
\textbf{Actor:} Lead User\\
\textbf{Flow:} Request cancellation → Calculate time to departure → Determine refund percentage → Atomically cancel all reservations → Update status → Process refund → Release seats.\\
\textbf{Rules:} BR-6, BR-21, BR-24, BR-25

\subsection{Administrative Use Cases}

\subsubsection*{UC-6: Create Flight Instance}
\textbf{Actor:} Flight Operations Manager\\
\textbf{Flow:} Select route → Validate source≠destination → Set departure/aircraft → Create SCHEDULED instance → Apply default multiplier → Load seat map.\\
\textbf{Rules:} BR-10, BR-11, BR-13, BR-14, BR-15, BR-17

\subsubsection*{UC-7: Update Dynamic Pricing}
\textbf{Actor:} Revenue Manager\\
\textbf{Flow:} Select flight instance → View multiplier/stats → Adjust multiplier (0.5-3.0 range) → Update for future bookings.\\
\textbf{Rules:} BR-17, BR-18

\subsubsection*{UC-8: Manage Route Pricing}
\textbf{Actor:} Pricing Analyst\\
\textbf{Flow:} Select route/class → View price history → Enter new price/validity → Enforce three-tier structure → Validate non-overlapping dates → Insert record.\\
\textbf{Rules:} BR-16, BR-18
...
\subsection{Detailed 3NF Compliance Analysis by Table}

\subsubsection{Iteration 1: Geographic and Airport Master Data Tables}

\paragraph{Table 1: Country}
\textbf{Attributes:} 
\begin{itemize}
    \item Country\_ID (NUMBER) - PK
    \item Country\_Name (VARCHAR2(50)) - Unique, NOT NULL
\end{itemize}
\textbf{Primary Key:} Country\_ID\\
\textbf{Candidate Keys:} Country\_Name (UNIQUE)\\
\textbf{3NF:} ✓ Atomic attributes, single PK (Country\_ID), Country\_Name depends only on PK, no transitive dependencies.

\paragraph{Table 2: State\_Province}
\textbf{Attributes:} 
\begin{itemize}
    \item State\_ID (NUMBER) - PK
    \item State\_Name (VARCHAR2(50)) - NOT NULL
    \item Country\_ID (NUMBER) - FK, NOT NULL
    \item UQ: (State\_Name, Country\_ID)
\end{itemize}
\textbf{Primary Key:} State\_ID\\
\textbf{Candidate Keys:} (State\_Name, Country\_ID)\\
\textbf{3NF:} ✓ Atomic attributes, single PK (State\_ID), all non-key attributes depend on PK, FK to Country is proper reference.

\paragraph{Table 3: City}
\textbf{Attributes:} 
\begin{itemize}
    \item City\_ID (NUMBER) - PK
    \item City\_Name (VARCHAR2(50)) - NOT NULL
    \item State\_ID (NUMBER) - FK, NOT NULL
    \item UQ: (City\_Name, State\_ID)
\end{itemize}
\textbf{Primary Key:} City\_ID\\
\textbf{Candidate Keys:} (City\_Name, State\_ID)\\
\textbf{3NF:} ✓ Atomic attributes, single PK (City\_ID), all non-key attributes depend on PK, FK to State is proper reference.

\paragraph{Table 4: Zip\_Master}
\textbf{Attributes:} 
\begin{itemize}
    \item Zipcode (VARCHAR2(10)) - PK
    \item City\_ID (NUMBER) - FK, NOT NULL
\end{itemize}
\textbf{Primary Key:} Zipcode\\
\textbf{Foreign Keys:} City\_ID → City(City\_ID)\\
\textbf{3NF:} ✓ Atomic attribute (City\_ID), single PK (Zipcode), depends only on PK, no transitive dependencies.

\paragraph{Table 5: Airport}
\textbf{Attributes:} 
\begin{itemize}
    \item Airport\_ID (VARCHAR2(10)) - PK (IATA Code: e.g., JFK, KHI)
    \item Airport\_Name (VARCHAR2(100)) - NOT NULL
    \item Zipcode (VARCHAR2(10)) - FK (optional, NULL allowed)
\end{itemize}
\textbf{Primary Key:} Airport\_ID\\
\textbf{Foreign Keys:} Zipcode → Zip\_Master(Zipcode) [nullable]\\
\textbf{3NF:} ✓ Atomic attributes, single PK (Airport\_ID), Airport\_Name depends on PK, optional Zipcode FK is proper dimensional reference.

\subsubsection{Iteration 2: Aircraft Configuration and Flight Route Tables}

\paragraph{Table 6: Aircraft\_Model}
\textbf{Attributes:} 
\begin{itemize}
    \item Model\_ID (VARCHAR2(20)) - PK (e.g., B737-800)
    \item Model\_Name (VARCHAR2(100)) - NOT NULL
    \item Manufacturer (VARCHAR2(50)) - Optional (NULL allowed)
\end{itemize}
\textbf{Primary Key:} Model\_ID\\
\textbf{3NF:} ✓ Atomic attributes, single PK (Model\_ID), all non-key attributes independent of each other, no transitive dependencies.

\paragraph{Table 7: Travel\_Class}
\textbf{Attributes:} 
\begin{itemize}
    \item Class\_ID (VARCHAR2(10)) - PK (e.g., ECO, BUS, FIR)
    \item Class\_Name (VARCHAR2(50)) - NOT NULL
\end{itemize}
\textbf{Primary Key:} Class\_ID\\
\textbf{3NF:} ✓ Atomic attribute (Class\_Name), single PK (Class\_ID), depends directly on PK, no transitive dependencies.

\paragraph{Table 8: Aircraft\_Row\_Class}
\textbf{Attributes:} 
\begin{itemize}
    \item Model\_ID (VARCHAR2(20)) - PK, FK
    \item Row\_Number (NUMBER) - PK
    \item Class\_ID (VARCHAR2(10)) - NOT NULL, FK
    \item CHECK: Row\_Number > 0
\end{itemize}
\textbf{Primary Key:} (Model\_ID, Row\_Number) - Composite\\
\textbf{Foreign Keys:} Model\_ID → Aircraft\_Model, Class\_ID → Travel\_Class\\
\textbf{3NF:} ✓ Non-key attribute (Class\_ID) depends on entire composite PK, no partial dependencies, FK is proper reference.

\paragraph{Table 9: Aircraft\_Seat\_Map}
\textbf{Attributes:} 
\begin{itemize}
    \item Model\_ID (VARCHAR2(20)) - PK, FK
    \item Row\_Number (NUMBER) - PK
    \item Seat\_Letter (CHAR(1)) - PK
    \item FK: (Model\_ID, Row\_Number) → Aircraft\_Row\_Class
\end{itemize}
\textbf{Primary Key:} (Model\_ID, Row\_Number, Seat\_Letter) - Composite\\
\textbf{Foreign Keys:} (Model\_ID, Row\_Number) → Aircraft\_Row\_Class\\
\textbf{3NF:} ✓ All columns are part of composite PK (no non-key attributes), junction table, trivially in 3NF.

\paragraph{Table 10: Flight\_Route}
\textbf{Attributes:} 
\begin{itemize}
    \item Route\_ID (VARCHAR2(10)) - PK
    \item Source\_Airport (VARCHAR2(10)) - FK, NOT NULL
    \item Dest\_Airport (VARCHAR2(10)) - FK, NOT NULL
    \item Base\_Duration (NUMBER) - Optional (NULL allowed, in minutes)
    \item CHECK: Source\_Airport ≠ Dest\_Airport
\end{itemize}
\textbf{Primary Key:} Route\_ID\\
\textbf{Foreign Keys:} Source\_Airport → Airport, Dest\_Airport → Airport\\
\textbf{3NF:} ✓ All non-key attributes depend on PK (Route\_ID), FK attributes are proper references, no transitive dependencies.

\subsubsection{Iteration 3: Services, Pricing, and Flight Operations}

\paragraph{Table 11: Flight\_Service}
\textbf{Attributes:} 
\begin{itemize}
    \item Service\_ID (VARCHAR2(10)) - PK (e.g., WIFI, MEAL)
    \item Service\_Name (VARCHAR2(50)) - NOT NULL
\end{itemize}
\textbf{Primary Key:} Service\_ID\\
\textbf{3NF:} ✓ Atomic attribute (Service\_Name), single PK (Service\_ID), no transitive dependencies.

\paragraph{Table 12: Service\_Offering}
\textbf{Attributes:} 
\begin{itemize}
    \item Route\_ID (VARCHAR2(10)) - PK, FK
    \item Class\_ID (VARCHAR2(10)) - PK, FK
    \item Service\_ID (VARCHAR2(10)) - PK, FK
    \item Valid\_From (DATE) - PK (temporal key)
    \item Valid\_To (DATE) - Optional (NULL means indefinitely valid)
    \item Is\_Complimentary (CHAR(1)) - Default 'Y'
    \item Cost\_If\_Paid (NUMBER(10,2)) - Default 0
    \item CHECK: Valid\_To IS NULL OR Valid\_To >= Valid\_From
\end{itemize}
\textbf{Primary Key:} (Route\_ID, Class\_ID, Service\_ID, Valid\_From) - Composite with Temporal Dimension\\
\textbf{Foreign Keys:} Route\_ID → Flight\_Route, Class\_ID → Travel\_Class, Service\_ID → Flight\_Service\\
\textbf{3NF:} ✓ All non-key attributes depend on entire composite PK with temporal dimension, FK attributes are proper references.

\paragraph{Table 13: Route\_Pricing}
\textbf{Attributes:} 
\begin{itemize}
    \item Pricing\_ID (NUMBER) - PK (Surrogate Key)
    \item Route\_ID (VARCHAR2(10)) - FK, NOT NULL
    \item Class\_ID (VARCHAR2(10)) - FK, NOT NULL
    \item Valid\_From (DATE) - NOT NULL
    \item Valid\_To (DATE) - NOT NULL
    \item Base\_Price (NUMBER(10,2)) - NOT NULL, CHECK >= 0
    \item UNIQUE: (Route\_ID, Class\_ID, Valid\_From)
    \item CHECK: Valid\_To >= Valid\_From
\end{itemize}
\textbf{Primary Key:} Pricing\_ID (Surrogate)\\
\textbf{Candidate Keys:} (Route\_ID, Class\_ID, Valid\_From)\\
\textbf{Foreign Keys:} Route\_ID → Flight\_Route, Class\_ID → Travel\_Class\\
\textbf{3NF:} ✓ Single surrogate PK with Candidate Key constraint, Base\_Price depends on PK representing unique route/class/period combination, FK attributes proper references.

\paragraph{Table 14: Flight\_Instance}
\textbf{Attributes:} 
\begin{itemize}
    \item Instance\_ID (VARCHAR2(20)) - PK (e.g., IAT1000-20251208)
    \item Route\_ID (VARCHAR2(10)) - FK, NOT NULL
    \item Model\_ID (VARCHAR2(20)) - FK, NOT NULL
    \item Departure\_Time (TIMESTAMP) - NOT NULL
    \item Arrival\_Time (TIMESTAMP) - NOT NULL
    \item Flight\_Status (VARCHAR2(20)) - Default 'SCHEDULED', CHECK IN (SCHEDULED, DELAYED, CANCELLED, LANDED)
    \item Price\_Multiplier (NUMBER(3,2)) - Default 1.00, CHECK > 0
\end{itemize}
\textbf{Primary Key:} Instance\_ID\\
\textbf{Foreign Keys:} Route\_ID → Flight\_Route, Model\_ID → Aircraft\_Model\\
\textbf{3NF:} ✓ All non-key attributes depend on PK (Instance\_ID), FK attributes are proper references, no transitive dependencies.

\paragraph{Table 15: App\_User}
\textbf{Attributes:} 
\begin{itemize}
    \item User\_ID (NUMBER) - PK (AUTO-GENERATED IDENTITY)
    \item Email (VARCHAR2(100)) - UNIQUE, NOT NULL
    \item Password\_Hash (VARCHAR2(256)) - NOT NULL (SHA256 hash, never plaintext)
    \item Phone\_Number (VARCHAR2(20)) - Optional (NULL allowed)
    \item Created\_At (TIMESTAMP) - Default SYSTIMESTAMP
    \item CHECK: Email LIKE '\%@\%.\_\%'
\end{itemize}
\textbf{Primary Key:} User\_ID (Surrogate)\\
\textbf{Candidate Keys:} Email (UNIQUE)\\
\textbf{3NF:} ✓ Atomic attributes, single surrogate PK (User\_ID), all non-key attributes independent, Email is Candidate Key.

\subsubsection{Iteration 4: Booking, Passenger, and Payment Records}

\paragraph{Table 16: Booking}
\textbf{Attributes:} 
\begin{itemize}
    \item Booking\_ID (VARCHAR2(20)) - PK (e.g., BK20251208001)
    \item Instance\_ID (VARCHAR2(20)) - FK, NOT NULL
    \item Lead\_User\_ID (NUMBER) - FK, Optional (NULL for guest bookings)
    \item Booking\_Date (TIMESTAMP) - Default SYSTIMESTAMP
    \item Total\_Price (NUMBER(12,2)) - NOT NULL, CHECK >= 0
    \item Booking\_Status (VARCHAR2(20)) - Default 'CONFIRMED', CHECK IN (CONFIRMED, CANCELLED, PENDING)
    \item Number\_Of\_Passengers (NUMBER(3)) - NOT NULL, CHECK > 0 AND <= 9
\end{itemize}
\textbf{Primary Key:} Booking\_ID\\
\textbf{Foreign Keys:} Instance\_ID → Flight\_Instance, Lead\_User\_ID → App\_User (optional)\\
\textbf{3NF:} ✓ All non-key attributes depend on PK (Booking\_ID), FK attributes are proper references, no transitive dependencies.

\paragraph{Table 17: Passenger}
\textbf{Attributes:} 
\begin{itemize}
    \item Passenger\_ID (NUMBER) - PK (AUTO-GENERATED IDENTITY)
    \item Title (VARCHAR2(10)) - CHECK IN ('Mr.', 'Mrs.', 'Ms.', 'Dr.', etc.)
    \item First\_Name (VARCHAR2(50)) - NOT NULL
    \item Middle\_Name (VARCHAR2(50)) - Optional
    \item Last\_Name (VARCHAR2(50)) - NOT NULL
    \item Date\_Of\_Birth (DATE) - NOT NULL
    \item Nationality (VARCHAR2(50)) - NOT NULL
    \item Linked\_User\_ID (NUMBER) - FK, Optional (NULL if not registered)
    \item UNIQUE: (First\_Name, Last\_Name, Date\_Of\_Birth)
\end{itemize}
\textbf{Primary Key:} Passenger\_ID (Surrogate)\\
\textbf{Candidate Keys:} (First\_Name, Last\_Name, Date\_Of\_Birth)\\
\textbf{Foreign Keys:} Linked\_User\_ID → App\_User (optional)\\
\textbf{3NF:} ✓ All non-key attributes depend on single surrogate PK (Passenger\_ID), Candidate Key provides alternate identifier, no transitive dependencies.

\paragraph{Table 18: Reservation}
\textbf{Attributes:} 
\begin{itemize}
    \item Reservation\_ID (NUMBER) - PK (AUTO-GENERATED IDENTITY)
    \item Booking\_ID (VARCHAR2(20)) - FK, NOT NULL
    \item Passenger\_ID (NUMBER) - FK, NOT NULL
    \item Instance\_ID (VARCHAR2(20)) - FK, NOT NULL
    \item Seat\_Row\_Number (NUMBER(3)) - Optional (NULL for lap infants)
    \item Seat\_Letter (CHAR(1)) - Optional (NULL for lap infants), CHECK IN ('A', 'B', 'C', 'D', 'E', 'F')
    \item Reservation\_Status (VARCHAR2(20)) - Default 'CONFIRMED', CHECK IN (CONFIRMED, CANCELLED, WAITLIST)
    \item Checked\_In (CHAR(1)) - Default 'N', CHECK IN ('Y', 'N')
    \item CONSTRAINT UQ\_Seat\_Instance UNIQUE (Instance\_ID, Seat\_Row\_Number, Seat\_Letter) WHERE Seat\_Row\_Number IS NOT NULL
\end{itemize}
\textbf{Primary Key:} Reservation\_ID (Surrogate)\\
\textbf{Foreign Keys:} Booking\_ID → Booking, Passenger\_ID → Passenger, Instance\_ID → Flight\_Instance\\
\textbf{3NF:} ✓ All non-key attributes depend on surrogate PK (Reservation\_ID), FK attributes are proper references, no transitive dependencies.

\paragraph{Table 19: Payment}
\textbf{Attributes:} 
\begin{itemize}
    \item Payment\_ID (NUMBER) - PK (AUTO-GENERATED IDENTITY)
    \item Booking\_ID (VARCHAR2(20)) - FK, NOT NULL
    \item Payment\_Method (VARCHAR2(20)) - CHECK IN ('CREDIT\_CARD', 'DEBIT\_CARD', 'BANK\_TRANSFER', 'CASH')
    \item Amount (NUMBER(12,2)) - NOT NULL, CHECK > 0
    \item Currency (VARCHAR2(3)) - Default 'PKR', CHECK IN ('PKR', 'USD', 'EUR')
    \item Payment\_Date (TIMESTAMP) - Default SYSTIMESTAMP
    \item Payment\_Status (VARCHAR2(20)) - Default 'COMPLETED', CHECK IN (COMPLETED, FAILED, REFUNDED, PENDING)
    \item Transaction\_ID (VARCHAR2(50)) - Optional (gateway reference)
\end{itemize}
\textbf{Primary Key:} Payment\_ID (Surrogate)\\
\textbf{Foreign Keys:} Booking\_ID → Booking\\
\textbf{3NF:} ✓ All non-key attributes depend on single surrogate PK (Payment\_ID), FK attribute is proper reference, no transitive dependencies.

\paragraph{Table 20: Cancellation\_Log}
\textbf{Attributes:} 
\begin{itemize}
    \item Cancellation\_ID (NUMBER) - PK (AUTO-GENERATED IDENTITY)
    \item Booking\_ID (VARCHAR2(20)) - FK, NOT NULL
    \item Cancellation\_Date (TIMESTAMP) - Default SYSTIMESTAMP
    \item Reason (VARCHAR2(500)) - Optional
    \item Refund\_Amount (NUMBER(12,2)) - NOT NULL, CHECK >= 0
    \item Refund\_Status (VARCHAR2(20)) - Default 'INITIATED', CHECK IN (INITIATED, PROCESSING, COMPLETED, FAILED)
\end{itemize}
\textbf{Primary Key:} Cancellation\_ID (Surrogate)\\
\textbf{Foreign Keys:} Booking\_ID → Booking\\
\textbf{3NF:} ✓ All non-key attributes depend on single surrogate PK (Cancellation\_ID), FK attribute is proper reference, no transitive dependencies.

\subsubsection{Iteration 5: User Relationships}

\paragraph{Table 21: User\_Family}
\textbf{Attributes:} 
\begin{itemize}
    \item User\_ID (NUMBER) - PK, FK
    \item Family\_User\_ID (NUMBER) - PK, FK
    \item Relationship (VARCHAR2(50)) - CHECK IN ('SPOUSE', 'CHILD', 'PARENT', 'SIBLING', 'OTHER')
    \item Added\_Date (TIMESTAMP) - Default SYSTIMESTAMP
    \item CHECK: User\_ID < Family\_User\_ID (prevents bidirectional duplicates)
\end{itemize}
\textbf{Primary Key:} (User\_ID, Family\_User\_ID) - Composite\\
\textbf{Foreign Keys:} User\_ID → App\_User, Family\_User\_ID → App\_User (self-referencing)\\
\textbf{3NF:} ✓ Non-key attributes depend on entire composite PK, FK attributes are proper references (self-referential), no transitive dependencies.
